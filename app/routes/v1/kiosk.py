...
kiosk = APIRouter(prefix="/application/api/v1/directory", tags=['Kiosk'])
kiosk_conn = KioskMall()
buy_kiosk_item = ProceedBuyItem()

@kiosk.get("/kiosk", response_class=JSONResponse, response_model=KioskResponse)
async def get_kiosk(response: Response,
    ...
    konteks = {"status": status.HTTP_203_NON_AUTHORITATIVE_INFORMATION,
               "total_records": kiosk_conn.total_items_in_kiosk, 
               "kiosk_items": showed_kiosk_detail}

    if not application_auth_token:
        response.headers['x_user_access_level'] = "Anonymous"
    else:
        konteks.update({"status": status.HTTP_202_ACCEPTED})
        response.headers['x_user_access_level'] = "User"
    return konteks
